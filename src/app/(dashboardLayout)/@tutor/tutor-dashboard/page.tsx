...
const TutorDashboardPage = async () => {
    ...
    const initials = user?.name?.split(" ").map((n) => n[0]).join("").slice(0, 2) || "TM";
        ...
        <div className="space-y-8 p-6 bg-background min-h-screen">
            ...
            <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4">
                ...
                <div className="flex items-center gap-2">
                    {profile?.isApproved ? (
                         <Badge variant="outline" className="border-green-600 text-green-600 px-3 py-1 bg-green-500/10">
            ...
            {!profile?.isApproved && (
                <Alert variant="destructive">
                    ...
                    <AlertDescription>
                        Your tutor profile is visible to you but hidden from students until admin approval.
                    </AlertDescription>
                </Alert>
            )}
            ...
            <div className="grid gap-6 md:grid-cols-7">
                    ...
                    <CardContent className="space-y-6">
                        ...
                        <div className="flex items-start gap-4">
                            ...
                            <div className="space-y-1">
                                ...
                                {profile?.isFeatured && (
                                    <Badge variant="secondary" className="mt-2 bg-yellow-500/10 text-yellow-500 hover:bg-yellow-500/20 border-yellow-500/20">
                        ...
                        {/* Subjects */}
                        <div className="space-y-2">
                            <h4 className="text-xs font-semibold text-muted-foreground uppercase tracking-wider">Subjects</h4>
                ...
                <Card className="col-span-3 h-fit bg-card text-card-foreground shadow-sm">
                    ...
                    <CardContent className="space-y-5">
                        <InfoRow label="Joined" value={new Date(user?.tutorProfile?.createdAt).toLocaleDateString()} />
                        <InfoRow label="Experience" value={`${profile?.experienceYears} Years`} />
                        <div className="flex justify-between items-center pb-2">
                            <span className="text-sm text-muted-foreground">Status</span>
                            <Badge variant={profile?.isApproved ? "outline" : "destructive"} className={profile?.isApproved ? "text-green-500 border-green-500" : ""}>
